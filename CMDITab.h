...
{
public:
    virtual void OnMDITabUnselectItem(int id, unsigned long item)
    {
    }
...
private:
    ...
    void RaiseOnUnselectItem(unsigned long id)
    {
        if (mCallback)
        {
            mCallback->OnMDITabUnselectItem(GetID(), id);
        }
    }
